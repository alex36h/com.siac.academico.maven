...
public class JIMoneda extends javax.swing.JInternalFrame {
    ...
    private void Save() {
        try {
            ...
            moneda.setNombre(this.txtNombre.getText());
            moneda.setDescripcion(this.txtDescrip.getText());
    ...
    private void Refrescar() {
        try {
            x = new JPACrud();
            model = Util.getModel(COL_MONEDA, false);
            Util.limpiarJTable(model);
            
            List<Moneda> monedas = x.buscarTodo(Moneda.class);
    ...
    private void Update()
    {
        if (jTable1.getSelectedRow() != -1) {
            obj = new Moneda();
                ...
                JDMoneda dialog = new JDMoneda(f, true,obj);
